# Sender's Mission:
# √ Take symmetric cipher algorithm to encrypt the message to ensure the confidentiality
# √ Use the (asymmetric) public key of receiver to encrypt the symmetric key to make the receipt undeniable
# √ Use MD5 algorithm to generate the abstract of the file to ensure the completeness of the message
# √ Use the sender's (asymmetric) private key to sign the encrypted abstract (mentioned above)
#   to make the sender's operations undeniable
# √ Pack up the encrypted message, private key (encrypted) and the signature and send to the receiver
...
class MD5(object):
    ...
    def group_processing(self):
        ...
        for i in range(0, 512, 32):
            num = ""
            # Get the hex value of every single section
...
# Function of message_encrypt, finished May 30 9:50 p.m. 2024
# The function will read the consented password document along with the original file and encrypt it
def message_encrypt():
    encrypted_lines = ""
...
def hash_abstract():
    with open("outbox/encrypted_msg.txt", "r") as sample_file:
        message = sample_file.read()
    message_hash = MD5(message)
    message_hash.fill_text()
    hash_result = message_hash.group_processing() + " "
    with open("outbox/encrypted_pwd_des.txt", 'r') as des_pwd_file:
        des_pwd = des_pwd_file.readline()
    password_hash = MD5(des_pwd)
    password_hash.fill_text()
    hash_result += password_hash.group_processing()
    print("Hash has been generated.")
    return hash_result
