...
# The function will generate and write the decrypted version of the received message.
class RSA:
...
def message_decrypt():
    ...
    with open('received/decrypted_msg.txt', 'w') as container_of_decrypted_contents:
...
def hash_decrypt():
    ...
    with open('received/hash_decrypted.txt', 'w') as decrypted_hash:
        decrypted_hash.write(str(encrypted_hash_chr))
    print("Hash Decrypted.")


def hash_check():
    with open('received/hash_decrypted.txt', 'r') as hash_decrypted:
